...
public class TikTokAudioTrack extends MpegTrack {
    ...
    protected Pair<String, String> loadPlaybackUrl() throws IOException {
        final TikTokAudioSourceManager.MetaData metdata = getSourceManager().extractData(
                trackInfo.author,
                trackInfo.identifier
        );

        return Pair.of(metdata.videoUrl, metdata.musicUrl);
    }

    @Override
    protected InternalAudioTrack createAudioTrack(AudioTrackInfo trackInfo, SeekableInputStream stream) {
        if (failedOnce && urlCache.getRight().contains(".mp3")) {
