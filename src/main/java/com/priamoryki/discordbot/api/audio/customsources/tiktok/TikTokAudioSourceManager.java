...
public class TikTokAudioSourceManager extends AbstractDuncteBotHttpSource {
    ...
    protected static final Pattern VIDEO_REGEX =
            Pattern.compile("^(https?://)?(www\\.)?tiktok\\.com/@(?<user>[^/]+)/video/(?<video>\\d+).*$");
    ...
    MetaData extractData(String userId, String videoId) throws IOException {
    ...
    protected MetaData extractData(String url) throws IOException {
        ...
        try (final CloseableHttpResponse response = getHttpInterface().execute(httpGet)) {
            ...
            final Matcher universalMatcher = UNIVERSAL_REGEX.matcher(html);

            if (universalMatcher.find()) {
                final JsonBrowser json = JsonBrowser.parse(universalMatcher.group(1).trim());
                final JsonBrowser video = json.get("__DEFAULT_SCOPE__").get("webapp.video-detail").get("itemInfo").get("itemStruct");
    ...
    protected static MetaData getMetaData(String url, JsonBrowser base) {
        ...
        final JsonBrowser musicJson = base.get("music");
